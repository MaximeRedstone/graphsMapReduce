...
def graphsDF(filestore_name: String): DataFrame = {
  ...
  def reduce_ccf(key: Int, values: ListBuffer[Int], acc: LongAccumulator): ListBuffer[(Int, Int)] = {
    val min_value = values.remove(values.indexOf(values.min))
    var ret = ListBuffer[(Int, Int)]()
    if (min_value < key) {
      ret += ((key, min_value))
      acc.add(values.length)
      for (i <- 0 until values.length) {
        ret += ((values(i), min_value))
      }
    }
...
//   val reducer = functions.udf.map(x, y => reduce_ccf(x, y, acc), MapType(IntegerType, IntegerType))
//   val reducer = udf {s: List[(Int, ListBuffer[Int])] => reduce_ccf(s(0)._1, s(0)._2, acc)}
//   val reducer = udf {(x: Int, y: ListBuffer[Int]) => reduce_ccf(x, y, acc)}: MapType(IntegerType, IntegerType)
  
  def reduce_ccf_udf(x: Int, y: ListBuffer[Int]): MapType(IntegerType, IntegerType) = { return reduce_ccf(x, y, acc) }
  val reducer = udf(reduce_ccf_udf)

//   val reducer = udf {s: Int => s}

  while (extracted_val != 0) {
    var previous = acc.value
...
//     print('Final output: ')
    ...
    var extracted_val = (current - previous).abs
    df.show()
    println(s"Final extract $extracted_val")
  }
